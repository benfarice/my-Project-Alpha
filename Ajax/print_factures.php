...
if(isset($_REQUEST['get_factures'])){
	...
	if(isset($_REQUEST['seller'])){
		$query_f = "select f.Num_facture as num_f,f.Total as t_f,f.Date_F as f_date from facture_vendeur f 
		where cast(f.Date_F as date) = '$searched_date'";
	}
	if(isset($_REQUEST['buyer'])){
		$query_f = "select f.CodeFac as num_f,f.Montant as t_f ,f.Date as f_date from factures_acht f 
	...
	while($reader_query__query_f = sqlsrv_fetch_array($result_query_query_f, SQLSRV_FETCH_ASSOC)){ 
		...
		?>
		 <tr>
			      ...
			      ?></td>
			      <td onclick="print_bill_func('<?php echo  $reader_query__query_f["num_f"] ?>');">
